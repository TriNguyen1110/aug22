...
POST /api/chat
  ...
  against the question text, reusing the existing proxy ingest path) before answering, since the
  user explicitly wants live-per-question fetches over cached-only. If Bright Data fails (e.g.
  account suspension) or returns nothing, `brightdata.ok` is false / `records_extracted` is 0 —
